...
const generateBobaCoordinates = () => generateBalls();

const getBobaStyle = (boba: BobaCoordinate): BobaStyle => ({
    ...
    >
      ...
      {bobaCoordinates.map((boba, index) => (
        <circle
          ...
          className={`${isAnimating ? `motion-safe:animate-boba-hard-shake` : isAnimationStopping ? `cls-boba-soft-shake` : ""} cls-boba-animation`}
        />
      ))}
