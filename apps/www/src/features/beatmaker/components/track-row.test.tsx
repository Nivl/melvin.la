...
describe(TrackRow, () => {
  ...
  it('active steps have aria-pressed="true"', () => {
    expect.assertions(2);
    const activeSteps = steps16.map((_, i) => i === 3);
    const { getAllByRole } = render(<TrackRow {...defaultProps} steps={activeSteps} />, {
      wrapper,
    });
    const stepBtns = getAllByRole("button").filter(
      (btn) => btn.getAttribute("aria-pressed") !== null,
  ...
  it("shows decode error when provided", () => {
    expect.assertions(1);
  ...
  it("clicking the drop zone label triggers file selection (calls onFileLoad via change)", async () => {
    expect.assertions(1);
    const user = userEvent.setup();
    const onFileLoad = vi.fn<(file: File) => void>();
    const file = new File(["audio"], "kick.mp3", { type: "audio/mpeg" });
    const { container } = render(<TrackRow {...defaultProps} onFileLoad={onFileLoad} />, {
      wrapper,
    });
