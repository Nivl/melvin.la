...
test.describe("UUID Tool", () => {
  ...
  test("generates a single v4 UUID", async ({ page }) => {
    ...
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/iv;
  ...
  test("can select v3 and see required fields appear", async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: /Name/v })).toBeVisible();
    await expect(page.getByRole("button", { name: /Namespace/v })).toBeVisible();

    // Verify count input is not visible for v3 (deterministic UUID)
    await expect(page.getByRole("textbox", { name: /How many to generate/v })).toBeHidden();
  });
  ...
  test("form fields appear/disappear based on UUID version selection", async ({ page }) => {
    ...
    await expect(page.getByRole("textbox", { name: /Name/v })).toBeHidden();
    await expect(page.getByRole("button", { name: /Namespace/v })).toBeHidden();
