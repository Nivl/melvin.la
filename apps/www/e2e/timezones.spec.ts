...
test.describe("Timezones Tool", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await expect(page).toHaveTitle(/Timezone converter/u);
  });
  ...
  test("can set initial city and target city", async ({ page }) => {
    ...
    const londonPill = page.locator('div[class*="bg-"]').filter({ hasText: /In London/u });
  ...
  test("can set multiple target cities from different countries", async ({ page }) => {
    ...
    const tokyoPill = page.locator('div[class*="bg-"]').filter({ hasText: /In Tokyo/u });
    const newYorkPill = page.locator('div[class*="bg-"]').filter({ hasText: /In New York/u });
    const sydneyPill = page.locator('div[class*="bg-"]').filter({ hasText: /In Sydney/u });
    ...
    const tokyoColor = tokyoClass?.match(/bg-\w+-\d+/u)?.[0];
    const newYorkColor = newYorkClass?.match(/bg-\w+-\d+/u)?.[0];
    const sydneyColor = sydneyClass?.match(/bg-\w+-\d+/u)?.[0];
    ...
    expect(tokyoText).toMatch(/\d{1,2}:\d{2}/u); // Contains time
    expect(newYorkText).toMatch(/\d{1,2}:\d{2}/u); // Contains time
    expect(sydneyText).toMatch(/\d{1,2}:\d{2}/u); // Contains time
  });
  ...
  test("shows different time differences for cities in different timezones", async ({ page }) => {
    ...
    const tokyoPill = page.locator('div[class*="bg-"]').filter({ hasText: /In Tokyo/u });
    const laPill = page.locator('div[class*="bg-"]').filter({ hasText: /In Los Angeles/u });
    ...
    expect(tokyoText).toMatch(/\d{1,2}:\d{2}/u);
