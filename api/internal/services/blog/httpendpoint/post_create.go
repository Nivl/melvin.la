...
func NewCreatePostInput(c *request.Context) (*CreatePostInput, error) {
	...
	if input.Title == "" {
		...
		return nil, httperror.NewValidationError("title", "title must be 100 chars or less")
	}

	if input.Slug != nil && len(*input.Slug) > 105 {
		return nil, httperror.NewValidationError("title", "title must be 105 chars or less")
	}

	if len(input.ContentJSON.Blocks) == 0 {
		return nil, httperror.NewValidationError("contentJson", "required")
	}
	...
	if input.Publish {
		if input.ThumbnailURL == nil || *input.ThumbnailURL == "" {
			...
			return nil, httperror.NewValidationError("thumbnailUrl", "title must be 255 chars or less")
		}
		if input.Description == nil || *input.Description == "" {
			return nil, httperror.NewValidationError("description", "required when publishing")
		} else if len(*input.Description) > 130 {
			return nil, httperror.NewValidationError("description", "title must be 130 chars or less")
		}
	}
