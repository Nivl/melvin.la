...
This directory is the root of the web monorepo.

- For any task under `web/`, read this file first.
...
- `web/` is not an application. It is the coordination layer for the web
  workspaces.
...
The workspace layout is defined in `web/pnpm-workspace.yaml`.
...
2. `web/AGENTS.md`
3. The nearest workspace-local `AGENTS.md`
...
- Run monorepo-wide commands from `web/`.
...
Use `web/` as the working directory for:
...
Shared tooling currently lives at the monorepo root:

- `web/package.json` keeps the root package intentionally small.
- `web/pnpm-workspace.yaml` defines the workspaces and root dependency catalog.
- `web/turbo.json` defines cross-workspace task names such as `lint`, `build`,
  `validate-code`, `test:unit`, and `test:e2e`.

Use the root when coordinating work across multiple workspaces. Use the local
workspace when running implementation-specific scripts.
...
- Make workspace-wide dependency coordination changes from `web/`.
