---
title: "Comment bien parler aux agents de codage IA"
excerpt: "La plupart des développeurs s'adressent aux agents de codage IA comme s'ils envoyaient un SMS. Voici la technique qui fonctionne vraiment — et pourquoi elle commence par demander à l'IA ce dont elle a besoin."
image: "cover.avif"
ogImage: "cover.png"
createdAt: "2026-02-26"
---

Voici comment la plupart des développeurs utilisent un agent de codage IA : ils ouvrent une fenêtre de chat et tapent quelque chose comme "ajoute un bouton de mode sombre à la page des paramètres". L'agent produit quelque chose. Ça fonctionne à peu près. Il y a des imperfections. Le développeur passe les vingt minutes suivantes à faire des allers-retours pour corriger les choses.

Ensuite, ils décident que le modèle n'est pas si bon.

Le modèle va très bien. C'est le prompt (l'invite) qui pose problème.

## La réalité inconfortable des prompts vagues

Chaque ambiguïté dans votre prompt est une décision que le modèle prend sans vous. La préférence pour le mode sombre doit-elle persister d'une session à l'autre ? Doit-elle respecter le paramètre au niveau de l'OS ? Où la préférence est-elle stockée — dans le localStorage, un profil utilisateur dans la base de données, un cookie ? Quelle bibliothèque de composants est utilisée et comment fonctionne la thématisation (theming) avec celle-ci ?

Vous ne l'avez pas précisé. Donc le modèle devine. Parfois il devine juste. Souvent non, et vous vous retrouvez avec un code qui fonctionne dans un environnement et plante dans un autre, ou un composant qui ignore la préférence existante de l'utilisateur, ou encore un bouton qui ne bascule rien du tout parce qu'il n'est pas connecté au bon contexte.
...
1. **Décrivez votre objectif de manière approximative** — juste assez pour que l'agent comprenne l'espace du problème
2. **Demandez-lui ce dont il aurait besoin pour le faire correctement** — "De quelles informations aurais-tu besoin pour bien accomplir cette tâche ? Quelles ambiguïtés dois-je clarifier ? Que devrais-je spécifier que je n'ai pas fait ?"
3. **Comblez les lacunes** — répondez à ses questions, fournissez le contexte qu'il a demandé
4. **Maintenant, confiez-lui la tâche** — soit vous intégrez ses questions dans votre prompt, soit vous lui demandez de réécrire votre demande initiale en une spécification complète, puis vous validez et exécutez.

Cette dernière étape est facultative mais très puissante. Vous pouvez demander à l'agent : "Sur la base de mes réponses, réécris ma demande initiale sous la forme d'une spécification de tâche complète." Ce que vous obtenez en retour, c'est un prompt que le modèle lui-même considère comme suffisamment complet pour bien s'exécuter. Ensuite, vous soumettez ce prompt comme la véritable tâche.

C'est ce que certains commencent à appeler l'ingénierie de prompt inversée (reverse prompt engineering) pour l'exécution des tâches — vous n'écrivez pas le prompt parfait de zéro, vous utilisez le modèle pour faire émerger ce qu'un prompt parfait devrait contenir.
...
Maintenant, l'agent a ce dont il a besoin. L'implémentation qu'il produit se connecte au bon fournisseur de thème, utilise l'API existante, correspond au modèle d'interface utilisateur en place et gère le repli (fallback) sur la préférence du système d'exploitation. Vous n'avez rien à corriger.

La différence dans la qualité du résultat n'est pas subtile.
...
Vous retournez voir la personne qui a signalé le bug, vous obtenez les réponses : c'est intermittent, cela n'arrive que lorsque l'utilisateur est sur la page depuis plus de dix minutes, aucune erreur dans la console, mais la requête réseau ne se déclenche jamais. C'est maintenant un bug complètement différent — probablement un problème d'expiration de token ou une closure obsolète (stale closure) dans un gestionnaire d'événements, pas un problème async/await.

Vous donnez cette information à l'agent, et maintenant il va au bon endroit. Il trouve un `useCallback` avec une dépendance obsolète qui capture un token d'authentification lors du rendu initial. Un correctif ciblé, pas de dommages collatéraux.
...
Il y a aussi un avantage secondaire : les questions que le modèle pose sont une liste de contrôle (checklist). Si vous utilisez systématiquement la même technique pour des tâches similaires — ajouter de nouveaux paramètres, corriger des bugs d'authentification, intégrer une nouvelle API — les questions commencent à vous sembler familières. Avec le temps, vous apprenez à inclure ces détails dans vos prompts sans qu'on vous le demande, et la qualité de base de vos prompts s'améliore.
...
La façon dont la plupart des développeurs interagissent avec les agents de codage privilégie la vitesse au détriment de la qualité. Une phrase rapide en entrée, un résultat médiocre en sortie, puis dix minutes de nettoyage — répété des dizaines de fois par jour.
...
Le meilleur prompt pour une tâche n'est pas celui que vous écrivez en trente secondes. C'est celui que le modèle vous dit dont il a besoin.
