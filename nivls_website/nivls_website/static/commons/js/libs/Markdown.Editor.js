...
(function () {
    ...
    Markdown.Editor = function (markdownConverter, idPostfix, help) {
        var defaultPostfix = {'bar': 'wmd-button-bar', 'preview': 'wmd-preview', 'input':'wmd-input'};
        var generatedDataPostfix = '';

        if (idPostfix === undefined || idPostfix === '') {
            idPostfix = defaultPostfix;
        } else if (Object.prototype.toString.call(idPostfix) == '[object String]') {
            var tmp = defaultPostfix;
            generatedDataPostfix = idPostfix;

            idPostfix = tmp.map(function(data) {
                data += idPostfix;
                return data;
            });
        }
            ...
            var makeButton = function (id, title, icon, textOp, group) {
                ...
                for (var i=0; i<icons.length; i++) {
                    if (icons[i]) {
                        ...
                        buttonImage.className = icons[i];
                        button.appendChild(buttonImage);
                    } else {
                        var spacing = document.createElement("span");
                        spacing.style.paddingRight = '6px';
                        button.appendChild(spacing);
                    }
                }
